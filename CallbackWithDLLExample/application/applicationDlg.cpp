...
BEGIN_MESSAGE_MAP(CapplicationDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_START, &CapplicationDlg::OnBnClickedButtonStart)
END_MESSAGE_MAP()
...
void UserCallback(int seconds)
{
	...
	CString output;
	output.Format(_T("%d seconds"), seconds);
	if (AfxMessageBox(output, MB_YESNO | MB_ICONQUESTION) != IDYES) {
		callbackwithdllexample::Stop();
	}
}


void CapplicationDlg::OnBnClickedButtonStart()
{
	callbackwithdllexample::Start();

	callbackwithdllexample::RegistCallback(UserCallback);
}
